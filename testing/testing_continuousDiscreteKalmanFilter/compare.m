...
estimates_c = csvread('output_filtered_data.txt');
estimates_c = estimates_c';
covariances_c = csvread('output_covariance_traces.txt');
covariances_cpp = cell(length(covariances)-1,1);

for i=1:length(covariances_cpp)
  covariances_cpp{i} = covariances_c(i);
end
...
disp('c++ continuous discrete Kalman filter matches matlab implementation')
