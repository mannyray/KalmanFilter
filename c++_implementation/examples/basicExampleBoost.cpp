...
int main(){
	...
	discreteDiscreteKalmanFilter<vectorBoost,matrixBoost> filter(initialTime, initialEstimate, initialCovariance,&ddfm);
	...
	outputfileEstimates.open("estimatesBoost.txt");
	for(int i = 0; i < steps; i++){
	...
	outputfileMeasurements.open("measurementsBoost.txt");
	for(int i = 0; i < steps; i++){
	...
	outputfileStates.open("statesBoost.txt");
	for(int i = 0; i < steps; i++){
